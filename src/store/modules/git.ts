...
const actions = {
  ...
  getGitRemoteRefs: async ({commit}: StoreActionContext<GitStoreState>, {id}: { id: string }) => {
    const res = await get(`${endpoint}/${id}/git/remote-refs`);
    commit('setGitRemoteRefs', res?.data || []);
    return res;
  },
  getGitBranches: async ({commit}: StoreActionContext<GitStoreState>, {id}: { id: string }) => {
    const res = await get(`${endpoint}/${id}/git/branches`);
    commit('setGitBranches', res?.data || []);
    return res;
  },
  getGitTags: async ({commit}: StoreActionContext<GitStoreState>, {id}: { id: string }) => {
    const res = await get(`${endpoint}/${id}/git/tags`);
    commit('setGitTags', res?.data || []);
    return res;
  },
  gitCheckout: async ({state}: StoreActionContext<GitStoreState>, {id, branch}: { id: string; branch: string }) => {
    const res = await post(`${endpoint}/${id}/git/checkout`, {branch});
    return res;
  },
  gitPull: async ({state}: StoreActionContext<GitStoreState>, {id}: { id: string }) => {
    const res = await post(`${endpoint}/${id}/git/pull`, {});
    return res;
  },
  gitCommit: async ({state}: StoreActionContext<GitStoreState>, {
    id,
    commit_message
